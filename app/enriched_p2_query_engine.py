...
class EnrichedP2QueryEngine:
    def __init__(self,base_dir):
        self.df=self._read(os.path.join(base_dir,"main_engine_enriched_p2.csv"))
    ...
    def _resolve(self,q):
        ...
        for r in self.sem.to_dict("records"):
            ...
            if score>0:
                hits.append((score,int(r.get("priority",0)),r.get("family",""),r["column_name"]))
        hits=sorted(hits,key=lambda x:(-x[0],-x[1],x[2],x[3]))
        out=[]; seen=set()
        for _,__,fam,c in hits:
            if c not in seen:
                seen.add(c); out.append((c,fam))
        return out
    def _extract_conditions(self,q,resolved):
        nq=_nrm(q); conds=[]; used=set()
        for c,_ in resolved:
            cc=_nrm(c)
            ...
            if m:
                ...
                used.add(c)
        if not conds:
            direct_flags=["is_low_activity_day","is_options_expiration_day","is_earnings_top_day","has_macro_event","has_macro_time","macro_impact_high","macro_impact_medium","macro_impact_low"]
            for c,_ in resolved:
                if c in direct_flags and _nrm(c) in nq:
                    conds.append((c,"=",1.0)); used.add(c)
        return conds,used
    def _apply(self,df,conds):
        out=df.copy()
        for c,op,v in conds:
        ...
        return out
    def _pick_metric(self,resolved,used):
        preferred=[(c,f) for c,f in resolved if c not in used and c in self.df.columns and pd.api.types.is_numeric_dtype(self.df[c])]
        if preferred: return preferred[0][0]
        defaults=["gold_daily_ret","oil_5m_ret","dxy_ret","vvix_ret","yield_curve_spread_value","ratio_spx_put_call_value","spx_qqq_corr20_value","spx_iwm_corr20_value","skew_index_value"]
        for c in defaults:
            if c in self.df.columns and c not in used: return c
        return None
    def answer(self,question,preview_rows=12):
        nq=_nrm(question)
        resolved=self._resolve(question)
        conds,used=self._extract_conditions(question,resolved)
        filt=self._apply(self.df,conds) if conds else self.df
        metric=self._pick_metric(resolved,used)
        out={"status":"OK","resolved_columns":[c for c,_ in resolved[:25]],"conditions":conds,"row_count_total":int(len(self.df)),"row_count_filtered":int(len(filt))}
        ...
        if any(x in nq for x in ["montre","show","liste","list","dates","quelles dates","which dates","preview","apercu","aperçu"]):
            keep=[c for c in ["date_key","time"]+[c for c,_ in resolved] if c in filt.columns]
            view=filt[keep].copy() if keep else filt.copy()
            out["answer_type"]="rows"; out["preview"]=_records(view,preview_rows); return out
        out["answer_type"]="summary"; out["metric"]=metric
        keep=[c for c in ["date_key","time"]+[c for c,_ in resolved] if c in filt.columns]
        view=filt[keep].copy() if keep else filt.copy()
